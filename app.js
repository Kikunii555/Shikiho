...
async function syncMasterData() {
  if (!supabaseClient) {
    showToast('⚠️ Supabaseが初期化されていません。config.jsonを設定してください。');
    return;
  }
  ...
  try {
    ...
    if (!spreadsheetId) {
      throw new Error('config.json に SPREADSHEET_ID が設定されていません。');
    }

    showToast('📊 スプレッドシートからマスタデータを取得中...');

    // URL自動判別とベースURL構築
    let baseUrl = '';
    if (spreadsheetId.includes('2PACX-') || spreadsheetId.includes('/d/e/')) {
      // ウェブ公開用スプレッドシートの場合
      let cleanId = spreadsheetId;
      if (spreadsheetId.includes('/d/e/')) {
        const match = spreadsheetId.match(/\/d\/e\/([^\/]+)/);
        if (match) cleanId = 'e/' + match[1];
      }
      cleanId = cleanId.replace(/\/pubhtml$/, '').replace(/\/pub$/, '');
      baseUrl = `https://docs.google.com/spreadsheets/d/${cleanId}/pub?output=csv`;
    } else {
      // 通常のスプレッドシート（共有設定済）の場合
      let cleanId = spreadsheetId;
      if (spreadsheetId.includes('/d/')) {
        const match = spreadsheetId.match(/\/d\/([^\/]+)/);
        if (match) cleanId = match[1];
      }
      baseUrl = `https://docs.google.com/spreadsheets/d/${cleanId}/gviz/tq?tqx=out:csv`;
    }

    // 1. Score_Settings 同期
    const ssRes = await fetch(`${baseUrl}&sheet=Score_Settings`);
    if (!ssRes.ok) throw new Error('Score_Settings シートのフェッチに失敗しました。');
    const ssText = await ssRes.text();
    const ssRows = parseCSV(ssText);
    const ssData = [];
    for (let i = 1; i < ssRows.length; i++) {
      const row = ssRows[i];
      if (row.length < 4 || !row[0]) continue;
      ssData.push({
        item_no: row[0].trim(),
        item_name: row[1].trim(),
        dividend_base_score: parseInt(row[2]) || 0,
        growth_base_score: parseInt(row[3]) || 0,
        description: row[4] ? row[4].trim() : ''
      });
    }
    if (ssData.length > 0) {
      const { error } = await supabaseClient.from('score_settings').upsert(ssData, { onConflict: 'item_no' });
      if (error) throw new Error('score_settings の同期に失敗: ' + error.message);
    }

    // 2. Rating_Criteria 同期
    const rcRes = await fetch(`${baseUrl}&sheet=Rating_Criteria`);
    if (!rcRes.ok) throw new Error('Rating_Criteria シートのフェッチに失敗しました。');
    const rcText = await rcRes.text();
    const rcRows = parseCSV(rcText);
    const rcData = [];
    for (let i = 1; i < rcRows.length; i++) {
      const row = rcRows[i];
      if (row.length < 3 || !row[0]) continue;
      
      const condition = row.slice(2).filter(x => x !== undefined).join(',').trim();
    ...
    if (rcData.length > 0) {
      const { error } = await supabaseClient.from('rating_criteria').upsert(rcData, { onConflict: 'id' });
      if (error) throw new Error('rating_criteria の同期に失敗: ' + error.message);
    }

    // 3. Brand_Master 同期
    const bmRes = await fetch(`${baseUrl}&sheet=Brand_Master`);
    if (!bmRes.ok) throw new Error('Brand_Master シートのフェッチに失敗しました。');
    const bmText = await bmRes.text();
    const bmRows = parseCSV(bmText);
    const bmData = [];
    for (let i = 1; i < bmRows.length; i++) {
      const row = bmRows[i];
      if (row.length < 2 || !row[0]) continue;
      const code = row[0].trim();
      if (code.length !== 4) continue; // 4文字以外の無効なコード（ヘッダーや空の行など）をスキップ
      bmData.push({
        code: code,
        name: row[1].trim(),
        market: row[2] ? row[2].trim() : '',
        industry: row[3] ? row[3].trim() : '',
        is_monitored: row[4] ? row[4].trim() === '1' || row[4].trim().toLowerCase() === 'true' : false
      });
    }
    ...
    const chunkSize = 500;
    let bmCount = 0;
    for (let i = 0; i < bmData.length; i += chunkSize) {
      const chunk = bmData.slice(i, i + chunkSize);
      const { error } = await supabaseClient.from('brand_master').upsert(chunk, { onConflict: 'code' });
      if (error) throw new Error('brand_master の同期に失敗: ' + error.message);
      bmCount += chunk.length;
    }

    await loadScoreSettings();

    showToast(`✅ マスタデータを正常に同期しました（${bmCount}銘柄更新）`);
  } catch (e) {
    console.error('Sync error:', e);
    showToast('❌ 同期中にエラーが発生しました: ' + e.message);
  } finally {
